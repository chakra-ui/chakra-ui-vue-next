import { toRefs, getCurrentInstance, computed, unref, ref, watch, watchEffect, onBeforeMount, onBeforeUnmount, defineComponent, mergeProps, reactive, onErrorCaptured, h, withDirectives, createVNode, isVNode } from 'vue';
import { useMultiStyleConfig, StylesProvider, chakra, useStyles, useTheme } from '@chakra-ui/vue-system';
import { useRef, getSelector, createContext } from '@chakra-ui/vue-utils';
...
import { hideOthers } from '@chakra-ui/vue-a11y';
...
function useModal(options) {
  ...
  var _toRefs = toRefs(options),
      modelValue = _toRefs.modelValue,
      id = _toRefs.id,
      closeOnOverlayClick = _toRefs.closeOnOverlayClick,
      closeOnEsc = _toRefs.closeOnEsc,
      initialFocusRef = _toRefs.initialFocusRef,
      finalFocusRef = _toRefs.finalFocusRef,
      useInert = _toRefs.useInert,
      blockScrollOnMount = _toRefs.blockScrollOnMount;

  var instance = getCurrentInstance();
  var finalFocusElement = computed(function () {
    var finalFocus;

    if (finalFocusRef != null && finalFocusRef.value) {
      var resolvedFinalFocusRef = isFunction(finalFocusRef.value) ? finalFocusRef.value == null ? void 0 : finalFocusRef.value() : finalFocusRef.value;
  ...
  var _useFocusLock = useFocusLock({
    ...
    immediate: true
      ...
      lock = _useFocusLock.lock,
      deactivate = _useFocusLock.deactivate;

  var _useBodyScrollLock = useBodyScrollLock(blockScrollOnMount != null && blockScrollOnMount.value ? modelValue : ref(false)),
      scrollLockRef = _useBodyScrollLock.scrollLockRef;
  /**
  ...
  var onKeyDown = function onKeyDown(event) {
    if (event.key === 'Escape') {
      ...
      if (closeOnEsc != null && closeOnEsc.value) {
        closeModal();
      }
 ...
 */
function useAriaHidden(node, shouldHide) {
  var undo = null;
...
var CModal = defineComponent({
  ...
  setup: function setup(props, _ref) {
    ...
    var styles = useMultiStyleConfig('Modal', mergeProps(props, attrs));
