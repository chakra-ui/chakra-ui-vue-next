...
title: Gradient
description:
  Learn how to use gradients in Chakra UI. Gradients are a way to transition between two or more colors.
version: 1.0+
---

Gradients are a way to have a transition between two or more colors. You can add
...
```vue live
<CBox w="100%" h="200px" bg-gradient="linear(to-r, green.200, pink.500)" />
```
...
```vue live
<CBox w="100%" h="200px" bg-gradient="linear(to-l, #7928CA, #FF0080)" />
```
...
<CBox
  w="100%"
  h="200px"
  bg-gradient="linear(to-r,gray.300,yellow.400,pink.200)"
/>
```
...
<CBox
  w="100%"
  h="200px"
  bg-gradient="linear(red.100 0%, orange.100 25%, yellow.100 50%)"
/>
```
...
<CBox
  ...
  font-weight="bold"
  border-radius="md"
  bg-gradient="linear(to-r, teal.500,green.500)"
