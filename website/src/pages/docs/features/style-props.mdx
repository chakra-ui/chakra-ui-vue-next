---
title: Style Props
description: The style props Chakra UI uses to let you style components
version: 1.0+
---
...
It helps to save time by providing helpful shorthand ways to style components.

Chakra UI uses [Styled System](https://styled-system.com/) to achieve this under the hood.
We extend the styled system's utilities to provide other helpful shorthands.

## Reference

The following table shows a list of every style prop and the properties within each group.
To learn more about it, you can also check the Styled System docs:
...
<template>
  ...
  <!-- 
    sets margin `8px` on all viewports
    and `12px` from the first breakpoint and up 
  -->
  <c-box :m="['2', '3']" />
</template>
...
<c-box w="100%" h="200px" bg-gradient="linear(to-t, green.200, pink.500)" />

<!-- adding radial gradient and color transitions -->
<c-box w="100%" h="200px" bg-gradient="radial(gray.300,yellow.400,pink.200)" />
...
<template>
  ...
  <c-text fontSize="2em" />
  <!-- 
    text-align `left` on all viewports 
    and `center` from the first breakpoint and up
  -->
...
| `flexDirection`, `flex-dir`, \*`direction` | `flex-direction`   | none      |
...
Asides from all the common style props listed above, all component will accept the
following props:
